...
export default class ProfileTab extends React.Component {
  ...
  renderOne = () => {
    return images.map(e => (
      <View key={e.id} 
        style={[
          { width: width/3 }, {height: width/3}, {marginBottom: 2},
          e.id % 3 !== 1 ? {paddingLeft: 2} : {paddingLeft: 0}
        ]} 
      >
        <Image source={e.source} style={{ flex: 1, width: undefined, height: undefined }} />
      </View>
    ))
  }
  ...
  renderSegment = () => {
    ...
    if(activeIndex === 2) {
      return (
        <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center', height: 350}} >
          <Image source={require('../../assets/profile.png')} style={{width: 150, height: 150}} />
          <Text style={{ fontSize: 24, fontWeight: '300', paddingBottom: 10 }} >Photos of You</Text>
    ...
    return (
      <Container style={styles.container} >
        <Header>
          ...
          <Body><Text style={styles.headerTitle} >tinhtran</Text></Body>
